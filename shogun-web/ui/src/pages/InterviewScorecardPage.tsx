                ...
                <tbody>
                  {rd.questionAnswers.map((qa, i) => (
                    <tr key={i} style={{ borderBottom: `1px solid ${BORDER}` }}>
                      <td style={{ padding: "0.4rem", verticalAlign: "top" }}><textarea value={qa.q} readOnly={rd.submitted} onChange={(e) => { if (rd.submitted) return; const u = [...rd.questionAnswers]; u[i] = { ...u[i], q: e.target.value }; updateRound(activeTab, { questionAnswers: u }); e.target.style.height = "auto"; e.target.style.height = e.target.scrollHeight + "px"; }} rows={1} style={{ ...inputStyle, fontSize: "0.78rem", resize: "none", fontFamily: "inherit", opacity: rd.submitted ? 0.8 : 1, overflow: "hidden", minHeight: "2.5rem" }} ref={(el) => { if (el) { el.style.height = "auto"; el.style.height = el.scrollHeight + "px"; } }} /></td>
                      <td style={{ padding: "0.4rem", verticalAlign: "top" }}><textarea value={qa.a} readOnly={rd.submitted} onChange={(e) => { if (rd.submitted) return; const u = [...rd.questionAnswers]; u[i] = { ...u[i], a: e.target.value }; updateRound(activeTab, { questionAnswers: u }); e.target.style.height = "auto"; e.target.style.height = e.target.scrollHeight + "px"; }} placeholder="Candidate response…" rows={1} style={{ ...inputStyle, fontSize: "0.78rem", resize: "none", fontFamily: "inherit", opacity: rd.submitted ? 0.8 : 1, overflow: "hidden", minHeight: "2.5rem" }} ref={(el) => { if (el) { el.style.height = "auto"; el.style.height = el.scrollHeight + "px"; } }} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
