...
export default function DocsViewer({ department }: DocsViewerProps) {
  ...
  return (
    <div className="flex h-full min-h-[28rem] flex-col overflow-hidden rounded-xl border border-surface-border bg-white dark:bg-slate-900 dark:border-slate-800">
      <div className="flex items-center justify-between border-b border-surface-border px-4 py-3 dark:border-slate-800">
        <h3 className="font-semibold text-slate-900 dark:text-slate-100">Department Documents</h3>
        <span className="text-xs text-slate-500 dark:text-slate-400">{docs.length} items</span>
      </div>

      <div className="grid flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[1fr_1.1fr]">
        <div className="overflow-y-auto border-b border-surface-border lg:border-b-0 lg:border-r dark:border-slate-800">
          {listQuery.isLoading && (
          ...
          {listQuery.isError && (
            <div className="m-4 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 dark:border-rose-900 dark:bg-rose-950 dark:text-rose-300">
              Failed to load documents.
            </div>
          )}
          {!listQuery.isLoading && docs.length === 0 && (
            <div className="m-4 rounded-lg border border-dashed border-slate-200 bg-slate-50 px-4 py-10 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-400">
              No documents yet.
            </div>
          )}
            ...
            {docs.map((doc, idx) => {
              ...
              return (
                <li key={doc.id || `${doc.name}-${idx}`}>
                  <button
                    type="button"
                    className={`flex w-full items-start gap-3 px-4 py-3 text-left hover:bg-surface-muted dark:hover:bg-slate-800 ${
                      active ? 'bg-brand-light/60 dark:bg-slate-800' : ''
                    }`}
        ...
        <div className="flex flex-col overflow-hidden bg-surface-muted dark:bg-slate-950/40">
          {!selected && (
          ...
          {selected && (
            <>
              <div className="flex items-center gap-2 border-b border-surface-border bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
                <div className="min-w-0 flex-1">
                  <div className="truncate font-semibold text-slate-900 dark:text-slate-100">{selected.name}</div>
              ...
              <div className="flex-1 overflow-auto p-4">
                {isImage && (
                  <img
                    ...
                    alt={selected.name}
                    className="mx-auto max-h-full max-w-full rounded-lg border border-surface-border bg-white object-contain shadow-sm dark:border-slate-700 dark:bg-slate-900"
                  />
                )}
                {isText && (
                  <div className="rounded-lg border border-surface-border bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                    ...
                    {previewQuery.isError && (
                      <div className="text-sm text-rose-600 dark:text-rose-400">Could not load preview.</div>
                    )}
                    {previewQuery.data && (
                      <pre className="whitespace-pre-wrap break-words text-xs text-slate-700 dark:text-slate-200">
                        {previewQuery.data}
                      </pre>
                    )}
                  </div>
                ...
                {!isImage && !isText && (
                  <div className="flex h-full flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-900">
