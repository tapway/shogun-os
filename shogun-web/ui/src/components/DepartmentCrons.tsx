...
export default function DepartmentCrons({ department }: DepartmentCronsProps) {
  ...
  const runNowMutation = useMutation({
    mutationFn: (cronId: string) => departmentsApi.runCron(department, cronId),
    onSuccess: () => {
      toast.success('Cron job triggered');
      queryClient.invalidateQueries({ queryKey: ['department-crons', department] });
    },
    onError: (err: Error) => toast.error(err.message || 'Failed to trigger cron'),
  });

  // Run history state
  const [historyCronId, setHistoryCronId] = useState<string | null>(null);
  const historyQuery = useQuery({
    queryKey: ['cron-history', department, historyCronId],
    queryFn: () => departmentsApi.getCronHistory(department, historyCronId!),
    enabled: !!historyCronId,
  });

  const handleRunNow = (cron: CronJob) => {
    runNowMutation.mutate(cron.id);
  };
  ...
  return (
    <div className="space-y-6 text-slate-900 dark:text-white p-2">
                    ...
                    <td className="px-4 py-6 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                          ...
                          title="Run Now"
                          className="flex items-center gap-1 rounded-lg bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 border border-emerald-500/30 transition-all shadow-sm disabled:opacity-50"
                        >
                          {runNowMutation.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5" />}
                          Run Now
                        </button>
                        <button
                          onClick={() => setHistoryCronId(historyCronId === c.id ? null : c.id)}
                          title="View Run History"
                          className={`flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-semibold border transition-all shadow-sm ${
                            historyCronId === c.id
      ...
      {/* Run History Panel */}
      {historyCronId && (
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 shadow-xl overflow-hidden">
          ...
          <div className="p-4">
            {historyQuery.isLoading ? (
              ...
              <p className="text-center text-xs text-slate-500 py-6">No runs recorded yet</p>
            ) : (
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {historyQuery.data.runs.map((run) => (
                  <div key={run.id} className="rounded-xl border border-slate-200 dark:border-slate-800 p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                        ...
                        {run.status === 'ok' ? '✓ Success' : run.status === 'running' ? 'Running…' : '✗ Failed'}
                      </span>
                    ...
                    {run.output && (
                      <pre className="text-[11px] text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-950 rounded-lg p-2 max-h-32 overflow-auto whitespace-pre-wrap font-mono leading-relaxed">
