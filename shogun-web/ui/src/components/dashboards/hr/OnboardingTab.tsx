...
export function OnboardingTab({ stats, color, department, onChanged }: Props) {
  ...
  const sectionNames = useMemo(
    () =>
      Array.from(
        new Set(checklistItems.map((i) => i.section).filter(Boolean)),
      ) as string[],
    [checklistItems],
  );
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Failed to update checklist item",
      );
    } finally {
      setBusy(false);
    }
  ...
  const removeItem = async (id: number, title: string) => {
    if (
      !window.confirm(
        `Remove checklist item "${title}"? All staff progress for this item will be removed too.`,
      )
    )
      return;
    setBusy(true);
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Failed to remove checklist item",
      );
    } finally {
      setBusy(false);
    }
    ...
    <div className="sd-stack">
      ...
      {editItem && (
        ...
        >
          ...
          >
            <h3
              style={{ margin: "0 0 1rem", fontSize: "1.05rem", color: TEXT }}
            >
              Edit Checklist Item
            </h3>
            <div
...
function FragmentRow({
  ...
  const staffProgress = progress.filter(
    (p) => p.staff_name === task.staff_name,
  );
    ...
    <>
      <tr
        ...
        style={{
          ...
          cursor: "pointer",
          background: expanded
            ? `color-mix(in srgb, ${LIME} 6%, transparent)`
            : undefined,
        }}
      >
        <td
        ...
        <td className="px-3 py-2" style={{ fontWeight: 600, color: TEXT }}>
          {task.staff_name || "—"}
        </td>
        <td className="px-3 py-2" style={{ color: MUTED }}>
          {task.department || "—"}
        </td>
        <td className="px-3 py-2" style={{ color: MUTED, fontSize: "0.78rem" }}>
        ...
        <tr style={{ borderBottom: `1px solid ${BORDER}` }}>
          <td
            ...
            style={{
              ...
              background: `color-mix(in srgb, ${LIME} 4%, transparent)`,
            }}
          >
            <div
              style={{
              ...
              >
                {sections.map((sec) => {
                  const secDone = sec.items.filter(
                    (i) =>
                      staffProgress.find((p) => p.item_id === i.id)?.completed,
                  ).length;
                  return (
                    <div key={sec.name}>
                      <div
                      ...
                      >
                        ...
                        <span
                          style={{
                            fontSize: "0.72rem",
                            color: secDone === sec.items.length ? OK : MUTED,
                          }}
                        >
                            ...
                            <label
                              ...
                              style={{
                                ...
                                background: completed
                                  ? `color-mix(in srgb, ${OK} 8%, transparent)`
                                  : "var(--samurai-bg)",
                                borderRadius: "0.5rem",
