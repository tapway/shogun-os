...
/** Official brand SVG icons — simple, recognizable, theme-aware */
  ...
  return (
    <div className="sd-stack">
        ...
        {stats.socialChannels.map((ch, i) => (
          <div key={i} className="sd-chart-card" style={{ textAlign: 'center', padding: '24px 16px' }}>
            ...
            {ch.followers !== undefined && (
              <div style={{ marginTop: 8, fontSize: '0.8rem', color: 'var(--samurai-muted)' }}>
      ...
      <div className="sd-chart-card" style={{ padding: '16px 20px' }}>
        <div style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: 4, color: 'var(--samurai-text)' }}>Next step</div>
        <div style={{ fontSize: '0.8rem', color: 'var(--samurai-muted)' }}>{stats.socialNextStep}</div>
      </div>
    </div>
  );
}
