...
**Goal:** Build a modular industry → department → skills onboarding system for Shogun OS, plus 24 new skills (27 total with 3 existing) that give any business an autonomous end-to-end e-commerce workflow across AutoCount, Shopee, Lazada, TikTok Shop, and Website (WooCommerce/Shopify/SiteGiant).

**Architecture:** Shogun OS replaces SiteGiant as the central hub. Users select their industry, then select departments, then the system auto-installs the right skills per department. When a selected department's skills depend on skills from other departments, the user chooses: add the full department, borrow just the needed skills, or skip. A master orchestrator sequences the full workflow with human approval gates. All operations are logged to a queryable audit trail.

**Tech Stack:** Python 3.8+ (stdlib only for connectors), Hermes Agent skills framework, existing Shogun OS profiles from `PROFILE_CATALOG.md`.

**Core Principles:**
1. One skill = one specific function. No mixing.
2. Skills are bundled per department — users pick departments, not individual skills.
3. E-commerce is a cross-industry add-on — any business can sell online, not just retail.
4. Cross-department dependencies are resolved during onboarding with 3 options: add / borrow / skip.

---

## Table of Contents

1. [Industry → Department → Skills Model](#1-industry--department--skills-model)
2. [Department Profiles](#2-department-profiles)
3. [Complete Skill Inventory — 27 Skills](#3-complete-skill-inventory--27-skills)
4. [Cross-Department Dependency Resolution](#4-cross-department-dependency-resolution)
5. [Onboarding Flow — Updated 5-Step Wizard](#5-onboarding-flow--updated-5-step-wizard)
6. [Department Manifest — Single Source of Truth](#6-department-manifest--single-source-of-truth)
7. [Shogun Master Data Store](#7-shogun-master-data-store)
8. [Skill Specifications — Full Build Details](#8-skill-specifications--full-build-details)
9. [Build Order & Dependencies](#9-build-order--dependencies)
10. [Skill Conventions](#10-skill-conventions)
11. [Client Requirement Coverage](#11-client-requirement-coverage)
12. [What to Build Beyond Skills](#12-what-to-build-beyond-skills)
...
### Key Decision: E-commerce is Cross-Industry

E-commerce is currently under "Retail" in `PROFILE_CATALOG.md`, but the 27 e-commerce skills are actually **cross-industry**. A manufacturer, a services company, or an F&B business could all sell on Shopee/Lazada/TikTok. E-commerce should be a **cross-industry add-on department**, not retail-only.

```
Industry: Manufacturing
Departments: [shared 8] + Production + Quality + Maintenance + Warehouse + HSE
Cross-industry add-on: ☑ E-commerce  ← manufacturer sells direct on Shopee too
```
...
| Department | Persona | Profile Slug | gbrain Source | Gateway Port | Nature |
|-----------|---------|-------------|---------------|-------------|--------|
| E-commerce | Denshi (電子 — "Digital") | `ecommerce-manager` | `ecommerce/` | 9111+ | Online store, marketplace ops, listings, orders, platform sync |
| Merchandising | Shohin (商品 — "Goods") | `merchandising-manager` | `merchandising/` | 9112+ | Buying, assortment, pricing, product performance |
| Marketing | Haiku (俳句) | `marketing-manager` | `marketing/` | 9105 (shared) | Brand, campaigns, content creation, creative assets |
| Compliance | Kata (型 — "Form") | `compliance-manager` | `compliance/` | 9106 (shared) | Standards, audits, policy, governance, approval gates |
...
### Skill → Department Mapping (27 skills across 4 departments)
...
> Product analysis by velocity and margin. These inform buying, assortment, and pricing decisions.
...
| Needed Skill | Home Department | Why Orchestrator Needs It | Required? |
|---|---|---|---|
| `product-velocity-analyzer` (17) | Merchandising | Step 2 — analyze sales velocity | Yes |
| `product-margin-analyzer` (18) | Merchandising | Step 3 — analyze margins | Yes |
| `promo-recommender` (19) | Marketing | Step 4 — generate promo recommendations | No (skip = no promo recs) |
| `cross-sell-bundle-recommender` (20) | Marketing | Step 5 — generate bundle recommendations | No (skip = no bundles) |
| `video-content-generator` (21) | Marketing | Step 6 — generate video scripts | No (skip = no video content) |
| `social-content-generator` (22) | Marketing | Step 7 — generate social content | No (skip = no social content) |
| `product-copy-generator` (23) | Marketing | Step 8 — generate product copy | No (skip = no product copy) |
| `banner-generator` (24) | Marketing | Step 9 — generate banners | No (skip = no banners) |
| `approval-gate` (26) | Compliance | Steps 4,9,10,11 — hold for approval | No (skip = no approval gates) |
| `action-audit-log` (27) | Compliance | Step 12 — log all actions | No (skip = no audit trail) |
...
When a skill is "borrowed" into another department, the skill file is identical — it doesn't change. What changes is **which profile directory it's installed in**:

```
Normal install:
  Skill: product-velocity-analyzer
  Home department: Merchandising (merchandising-manager)
  Installed to: ~/.hermes/profiles/merchandising-manager/skills/

Borrowed install:
  Skill: product-velocity-analyzer
  Home department: Merchandising (for catalog reference)
  Installed to: ~/.hermes/profiles/ecommerce-manager/skills/
  Borrowed by: ecommerce-manager
```

The `install-to-profiles.py` script already supports this — it's just a different `--profiles` argument:

```bash
# Normal: install to home department
python3 skills/shogunify/scripts/install-to-profiles.py \
  --skill product-velocity-analyzer --profiles merchandising-manager --force

# Borrowed: install to e-commerce profile
python3 skills/shogunify/scripts/install-to-profiles.py \
  --skill product-velocity-analyzer --profiles ecommerce-manager --force
```
...
### How the Orchestrator Handles "Skipped" Dependencies

If the user chose "Skip" for some skills, the orchestrator adapts at runtime:

```python
# Inside workflow_orchestrator.py
STEPS = [
    {"step": 1, "skill": "autocount-product-sync", "required": True},
    {"step": 2, "skill": "product-velocity-analyzer", "required": True, "home_dept": "merchandising"},
    {"step": 3, "skill": "product-margin-analyzer", "required": True, "home_dept": "merchandising"},
    {"step": 4, "skill": "promo-recommender", "required": False, "home_dept": "marketing"},
    {"step": 5, "skill": "cross-sell-bundle-recommender", "required": False, "home_dept": "marketing"},
    {"step": 6, "skill": "video-content-generator", "required": False, "home_dept": "marketing"},
    {"step": 7, "skill": "social-content-generator", "required": False, "home_dept": "marketing"},
    {"step": 8, "skill": "product-copy-generator", "required": False, "home_dept": "marketing"},
    {"step": 9, "skill": "banner-generator", "required": False, "home_dept": "marketing"},
    {"step": 10, "skill": "price-sync", "required": False, "home_dept": "ecommerce"},
    {"step": 11, "skill": "listing-sync", "required": False, "home_dept": "ecommerce"},
    {"step": 12, "skill": "action-audit-log", "required": False, "home_dept": "compliance"},
]

def run_full(self):
    for step in STEPS:
        if not self._skill_available(step["skill"]):
            if step["required"]:
                return {"success": False, "error": f"Required skill {step['skill']} not installed"}
            else:
                self._log_skip(step)
                continue
...
## 5. Onboarding Flow — Updated 5-Step Wizard
...
1. Reads `department-manifest.yaml`
2. Creates Hermes profiles for each selected department (`hermes profile create <slug>`)
3. Generates SOUL.md per profile (`python3 scripts/generate-profile.py`)
4. Installs skills per profile (`install-to-profiles.py --skill <name> --profiles <slug>`)
5. For borrowed skills: installs to the borrowing profile, not the home profile
6. Assigns gateway ports
7. Creates gbrain sources
8. Wires cron jobs
9. Verifies with `verify-install.sh`

---

## 6. Department Manifest — Single Source of Truth

The `department-manifest.yaml` file is the single source of truth for the onboarding wizard, `generate-profile.py`, and `install-to-profiles.py`. It replaces scattered skill lists in `PROFILE_CATALOG.md`.

```yaml
# department-manifest.yaml
# Single source of truth: industries → departments → skills → dependencies
# Read by: onboarding wizard, generate-profile.py, install-to-profiles.py

industries:
  retail:
    departments:
      - ecommerce        # cross-industry add-on (available to all industries)
      - merchandising
      - stores
      - crm-loyalty
      - supply-chain
...
cross_industry_addons:
  - ecommerce             # available to any industry
  - coding                # available to any industry

departments:
  ecommerce:
    persona: "Denshi"
    kanji: "電子"
    profile_slug: "ecommerce-manager"
  ...
  merchandising:
    persona: "Shohin"
    kanji: "商品"
    profile_slug: "merchandising-manager"
  ...
  marketing:
    persona: "Haiku"
    kanji: "俳句"
    profile_slug: "marketing-manager"
    gbrain_source: "marketing/"
    gateway_port: 9105
    skills: []              # base marketing skills (brand, campaigns)
    addon_skills:           # e-commerce add-on skills (installed only when E-commerce is selected)
      - promo-recommender
  ...
  compliance:
    persona: "Kata"
    kanji: "型"
    profile_slug: "compliance-manager"
    gbrain_source: "compliance/"
    gateway_port: 9106
    skills: []              # base compliance skills (audits, policy)
    addon_skills:           # e-commerce add-on skills
      - approval-gate
      - action-audit-log

# Dependencies: when a skill in one department needs skills from another
dependencies:
  ecommerce-workflow-orchestrator:
    needs:
      - skill: product-velocity-analyzer
        ...
        required: true
        step: 2
        step_name: "Analyze sales velocity"
      - skill: product-margin-analyzer
        from: merchandising
        required: true
        step: 3
        step_name: "Analyze product margins"
      - skill: promo-recommender
        from: marketing
        required: false
        step: 4
        step_name: "Generate promo recommendations"
        skip_message: "Workflow will skip promo recommendation generation"
      - skill: cross-sell-bundle-recommender
        from: marketing
        required: false
        step: 5
        step_name: "Generate bundle recommendations"
        skip_message: "Workflow will skip bundle recommendation generation"
      - skill: video-content-generator
        from: marketing
        required: false
        step: 6
        step_name: "Generate video content"
        skip_message: "Workflow will skip video content generation"
      - skill: social-content-generator
        from: marketing
        required: false
        step: 7
        step_name: "Generate social content"
        skip_message: "Workflow will skip social content generation"
      - skill: product-copy-generator
        from: marketing
        required: false
        step: 8
        step_name: "Generate product copy"
        skip_message: "Workflow will skip product copy generation"
      - skill: banner-generator
        from: marketing
        required: false
        step: 9
        step_name: "Generate banners"
        skip_message: "Workflow will skip banner generation"
      - skill: approval-gate
        from: compliance
        required: false
        step: "4,9,10,11"
        step_name: "Hold for human approval"
        skip_message: "Workflow will run without approval gates — all actions execute immediately"
      - skill: action-audit-log
        from: compliance
        required: false
        step: 12
        step_name: "Log all actions"
        skip_message: "Workflow will run without audit logging"
```

---

## 7. Shogun Master Data Store

All data ingestion skills (7-8) write to a unified local data store. All listing-sync and price-sync skills (9-16) read from this store. Analysis skills (17-18) read from this store. Content skills (21-23) read product data from this store.
...
└── sku-mapping.json        # SKU mapping across platforms
```
...
## 8. Skill Specifications — Full Build Details

### What Already Exists (DO NOT Rebuild)

**`shopee-connector` (✅ Existing)** — Shopee Open Platform API v2, HMAC-SHA256 signing, stdlib only. Methods: `connect()`, `read_orders(status, since)`, `read_products()`, `update_listing(product_data)`, `read_analytics(period)`, `read_returns()`. Env: `SHOPEE_PARTNER_ID`, `SHOPEE_API_KEY`, `SHOPEE_ACCESS_TOKEN`, `SHOPEE_SHOP_ID`.

**`lazada-connector` (✅ Existing)** — Lazada Open Platform (LOP), custom HMAC-SHA256 signing, stdlib only. Methods: `connect()`, `read_orders(status, since)`, `read_products()`, `update_product(data)`, `read_finance()`, `read_seller_performance()`. Env: `LAZADA_APP_KEY`, `LAZADA_APP_SECRET`, `LAZADA_ACCESS_TOKEN`, `LAZADA_SELLER_ID`.

**`autocount-connector` (✅ Existing)** — AutoCount AOTG, RESTful JSON, Bearer auth. Methods: `connect()`, `read_stock_balance(sku)`, `read_sales_invoices(since)`, `read_debtor_aging()`, `read_purchase_orders(status)`, `write_sales_invoice(data)`, `write_stock_adjustment(data)`. Env: `AUTOCOUNT_API_URL`, `AUTOCOUNT_API_KEY`, `AUTOCOUNT_COMPANY_DB`.
...
1. Create directories: `retail/action-audit-log/`, `retail/action-audit-log/scripts/`
2. Write `SKILL.md`:
...
**Total: 24 new skills, ~55 files, 9 phases, 4 departments.**

---

## 10. Skill Conventions
...
5. **Category:** `retail` for all 24 skills (skill category groups skills in the catalog; department assignment controls which profile runs them)
...
11. **Master store location:** `~/.hermes/ecommerce/master/`
12. **Logs location:** `~/.hermes/ecommerce/logs/`
13. **Approvals location:** `~/.hermes/ecommerce/approvals/`
14. **Banners location:** `~/.hermes/ecommerce/banners/pending/` and `approved/`
15. **Workflow state:** `~/.hermes/ecommerce/workflow-state.json`
16. **Department installation:** Each skill installed to its assigned department profile via `python3 skills/shogunify/scripts/install-to-profiles.py --skill <name> --profiles <profile-slug> --force`
17. **Borrowed skills:** When a skill is borrowed into another department, it's installed to the borrowing profile's directory — the skill file doesn't change, only the install target
18. **SKILL.md frontmatter `depends_on`:** New skills declare their cross-department dependencies in frontmatter so the onboarding wizard can read them dynamically

---

## 11. Client Requirement Coverage
...
| 7. Workflow Automation (10-step sequence) | 25 |
| 8. Human Approval Workflow | 26 |
| 9. Logging & Monitoring | 27 |
| 10. Future Expansion (modular architecture) | All skills are atomic — new ones can be added without modifying existing. Industry → department → skills model scales to any future industry. |

---

## 12. What to Build Beyond Skills

The industry → department → skills model requires infrastructure beyond the 24 skills themselves:

| # | Artifact | Purpose | Depends On |
|---|---------|---------|------------|
| 1 | `department-manifest.yaml` | Single source of truth: industries → departments → skills → dependencies | — |
| 2 | Onboarding wizard update (UI) | 5-step flow with dependency resolution screen | Manifest (1) |
| 3 | `generate-profile.py` update | Read manifest, create profiles with correct skills | Manifest (1) |
| 4 | `install-to-profiles.py` update | Handle "borrow" mode (install skill to non-home profile) | Manifest (1) |
| 5 | `ecommerce-workflow-orchestrator` runtime dependency check | Check skill availability at runtime, skip missing steps gracefully | Skill 25 |
| 6 | `SKILL.md` frontmatter convention | Add `depends_on:` field to skill frontmatter | All skills |
...
| Industry | Departments | E-commerce Add-on? |
|----------|------------|-------------------|
| F&B | Kitchen, Menu, Delivery, Hygiene | ✅ Sell meal kits on Shopee |
| Healthcare | Clinic, Pharmacy, Patient Records | ✅ Sell health products on Shopee |
| Construction | Site Ops, Safety, Materials, Equipment | ✅ Sell building supplies online |
| Education | Admissions, Curriculum, Student Records | ✅ Sell courses/materials online |

### Future E-commerce Skills

| Future Capability | Department | Approach |
|-------------------|------------|----------|
| Purchase reorder proposals | E-commerce | New skill: `reorder-proposal-generator` |
| Inventory optimisation | Merchandising | New skill: `inventory-optimiser` |
| Dead stock clearance planning | Merchandising | New skill: `clearance-plan-generator` |
| Affiliate campaign generation | Marketing | New skill: `affiliate-campaign-generator` |
| Customer service automation | Compliance or Customer Support | New skill: `customer-service-bot` |
| Supplier communication | E-commerce | New skill: `supplier-comms` |
| Sales reporting | Merchandising | New skill: `sales-report-generator` |
| Dashboard and KPI monitoring | E-commerce | Extend Shogun web portal |
| Automatic campaign scheduling | Marketing | New skill: `campaign-scheduler` |

### How Future Skills Declare Dependencies

When a new skill is built, its SKILL.md frontmatter includes a `depends_on:` field. The onboarding wizard reads all skill frontmatter to build the dependency graph dynamically:

```yaml
---
name: reorder-proposal-generator
description: "Generate purchase reorder proposals from stock velocity data."
version: 1.0.0
tags: [retail, ecommerce, procurement, reorder]
triggers: ["reorder", "replenish", "purchase proposal"]
depends_on:
  - skill: product-velocity-analyzer
    from: merchandising
    required: true
  - skill: autocount-product-sync
    from: ecommerce
    required: true
---
```

The manifest + frontmatter `depends_on` together form a complete dependency graph that the onboarding wizard traverses to show users exactly what they need.
