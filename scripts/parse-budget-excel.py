...
def update_examples_mock(mock_path: str, bva_line_items: list[dict]) -> None:
    """Update examples/finance-budget.json with new bvaLineItems, preserving other mock data."""
    ...
    # Replace bvaDepartments with bvaLineItems
    mock = data.get("dashboard_mock", {})
    mock.pop("bvaDepartments", None)
    mock["bvaLineItems"] = bva_line_items
    data["dashboard_mock"] = mock

    # Also update the canonical lines + budget_baseline
    ...
    print(f"Updated {mock_path} with {len(bva_line_items)} P&L line items")
...
def main():
    ...
    print(f"   Actual source: QBO (mock uses Excel column B YTD actuals for dev)")
